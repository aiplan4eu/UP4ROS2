...
#include "plansys2_upf_domain_expert/UPFDomainExpertClient.hpp"
...
TEST(upf_domain_expert, lifecycle)
{
  auto test_node = rclcpp::Node::make_shared("get_action_from_string");
  auto domain_node = std::make_shared<plansys2::UPFDomainExpertNode>();
  auto domain_client = std::make_shared<plansys2::UPFDomainExpertClient>();

  std::string pkgpath = ament_index_cpp::get_package_share_directory("plansys2_upf_domain_expert");
...
TEST(upf_domain_expert, lifecycle_error)
{
  auto test_node = rclcpp::Node::make_shared("get_action_from_string");
  auto domain_node = std::make_shared<plansys2::UPFDomainExpertNode>();
  auto domain_client = std::make_shared<plansys2::UPFDomainExpertClient>();
